...
import { petsjson } from "../assets/flyff/pets.js";
import { cardsjson } from "../assets/flyff/cards.js";
...
import { premiumbonusjson } from "../assets/flyff/premiumBonus.js";
...
export class Utils {
    ...
    static pets = petsjson;
    static petes = petsjson;
    static petds = petsjson;
    static petcs = petsjson;
    static petbs = petsjson;
    static petas = petsjson;
    static petxs = petsjson;
    static cards = cardsjson;
    static upgradeBonus = upgradesjson;
    static bonusStats = premiumbonusjson;
    ...
    static getWeapPiercingCards() { return this.cards.filter(card => card.subcategory == "weappiercingcard" && card.abilities); }
    ...
    static getPets() { return this.pets.filter(pet => pet.grade < 2 && pet.abilities); }
    static getPetes() { return this.petes.filter(pete => pete.grade < 3 && pete.abilities); }
    static getPetds() { return this.petds.filter(petd => petd.grade < 4 && petd.abilities); }
    static getPetcs() { return this.petcs.filter(petc => petc.grade < 5 && petc.abilities); }
    static getPetbs() { return this.petbs.filter(petb => petb.grade < 6 && petb.abilities); }
    static getPetas() { return this.petas.filter(peta => peta.grade < 8 && peta.abilities); }
    static getPetxs() { return this.petxs.filter(petx => petx.grade < 10 && petx.abilities); }
    static getBonusStats() { return this.bonusStats.filter(bonusStat => bonusStat.category == "stats" && bonusStat.abilities); }
